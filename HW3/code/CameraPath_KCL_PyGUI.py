...
cmds.text("",w=5)
cmds.button(label="Add", p="targetObjLayout", w=40, command=ft(bf.addTargetObj, targetObj))
cmds.button(label="Remove", p="targetObjLayout", w=55, command=ft(bf.removeTargetObj, targetObj))
...
cmds.text("",w=5)
cmds.button(label="Add", p="curveLayout", w=40, command=ft(bf.addCurve, targetCurve))
cmds.button(label="Remove", p="curveLayout", w=55, command=ft(bf.removeCurve, targetCurve))
...
cmds.button(label="Place Camera", p="cameraPlaceLayout", w=97,
            command=ft(bf.placeCamera, distance,targetObj,cameraPlace,list_cameraPlace))
...
cmds.button(label="Follow Path", p="keyframeLayout", w=97,
            command=ft(bf.followPath, targetObj,targetCurve,keyframes,intervals))
