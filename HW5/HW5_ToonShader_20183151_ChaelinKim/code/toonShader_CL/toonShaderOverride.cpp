...
MHWRender::MPxSurfaceShadingNodeOverride* toonShaderOverride::creator(const MObject& obj)
{
	return new toonShaderOverride(obj);
}

toonShaderOverride::toonShaderOverride(const MObject& obj)
...
toonShaderOverride::~toonShaderOverride()
{
}

MHWRender::DrawAPI toonShaderOverride::supportedDrawAPIs() const
{
	// works in both gl and dx
	return MHWRender::kOpenGL | MHWRender::kDirectX11 | MHWRender::kOpenGLCoreProfile;
}

MString toonShaderOverride::fragmentName() const
{
...
void toonShaderOverride::getCustomMappings(
	MHWRender::MAttributeParameterMappingList& mappings)
{
	...
	// Our toon only uses a single float for specularity, while the Maya
	// toon fragment uses a full 3-float color. We could add a remap fragment
...
MString toonShaderOverride::primaryColorParameter() const
{
	// Use the color parameter from the toon fragment as the primary color
	return "color";
}

MString toonShaderOverride::bumpAttribute() const
{
	// Use the "normalCamera" attribute to recognize bump connections
	return "normalCamera";
}

void toonShaderOverride::updateDG()
{
	MStatus status;
...
void toonShaderOverride::updateShader(
